# Tazeleme — 2026-08-08

Katman: `hepsi` · Veri dizini: `ktpanel`


### XK100 ağırlıkları — ✓ GEÇTİ
- ✓ **kapsam**: 100/100 (%100)
- ✓ **tarih birliği**: tek tarih: 2026-08-06
- ✓ **toplam**: 100.00 (hedef 100 ±3)

### XKTUM ağırlıkları — ✓ GEÇTİ
- ✓ **kapsam**: 150/150 (%100)
- ✓ **tarih birliği**: tek tarih: 2026-08-06
- ✓ **toplam**: 96.50 (hedef 96.5 ±3)

### XKTMT ağırlıkları — ✓ GEÇTİ
- ✓ **kapsam**: 34/34 (%100)
- ✓ **tarih birliği**: tek tarih: 2026-08-06
- ✓ **toplam**: 100.00 (hedef 100 ±3)

### Multiple fiyatları — ✓ GEÇTİ
- ✓ **kapsam**: 141/141 (%100)
- ✓ **tarih birliği**: tek tarih: 2026-08-06
- ✓ **aykırı değer**: temiz (sınır ±%25)

### Model sicili — ✓ GEÇTİ
- ✓ **kapsam**: 40/40 (%100)
- ✓ **tarih birliği**: tek tarih: 2026-08-06
- ✓ **aykırı değer**: temiz (sınır ±%25)

### Risk metrikleri — ✓ GEÇTİ
- ✓ **kapsam**: 141/141 (%100)
- ✓ **aykırı değer**: temiz (sınır ±%3)
- ✓ **aykırı değer**: temiz (sınır ±%150)
- ℹ **beta referansı: XU100.IS** (251 gün)
- ℹ 5 gün kurumsal işlem süzgecine takıldı (±%20 üstü hareket — bölünme/bedelsiz) — ⚠ XKTUM bulunamadı, YEDEK endeks kullanıldı; sicil karşılaştırmasıyla taban FARKLI olabilir
- denenen: arşiv XKTUM: 61 nokta (son 120g: 7) · XKTUM.IS: boş · ^XKTUM: boş · XU100.IS: 251

### TEFAS köprü (bilgi)
- getiri: 1048 fon ✓ · liste: 1032 kayıt · alanlar: fonKod, unvan, kurucuKod, kurucuAd, oprKod, oprAd, durum, tarih

### TEFAS çekim tanısı (bilgi)
- yol: yok
- yakalanan JSON: 0 / toplam yanıt: 1 · sayfa: "Request Rejected" · gövde: `The requested URL was rejected. Please consult with your administrator. Your support ID is: <9051587007834147722> [Go Back]`

### Katılım fonları — ✓ GETİRİ-MODU: 36/46 fonun 5 dönem getirisi + fiyat/1G yazıldı (AUM/akış önceki turdan)

### Bilanço tetiği — ✓ 69 şirket FR yayımladı
- AKMGY, AKSUE, ALCTL, ALGYO, ARFYE, BAYRK, BJKAS, BLCYT, BNPFK, BOSSA, BRSAN, CAGFA, CEMTS, CMBTN, CMENT, CRFSA, DESPC, DFKTR, DOFER, ECILC …

### Endeks üyelikleri — ✓ XK030EA:30 · XKTUM:242 · XK100:100 · XK050:50 · XK030:30 · XSRDK:24 · XKTMT:39
- ✓ üyelik uyumu: panelin 150 hissesi resmî listede (kapsam %96.5, tasarım gereği ilk 150)

### Endeks kapanışları — ✓ 87 endeks · veri günü 2026-08-07
- XKTUM 18694.71 · XKTMT 17201.71 · XK100 17517.53 · XU100 13779.39 · BISTTLREFK 4010.70089
- arşiv: 490 gün · dosyalar: bisttlrefkendeksi.csv(1), zip:[FiyatEndeksleri_PriceIndices.csv, GetiriEndeksleri_ReturnIndices.csv], FiyatEndeksleri_PriceIndices.csv(84), GetiriEndeksleri_ReturnIndices.csv(2)
- ⚠ inmeyenler: FiyatEndeksleri_PriceIndices.csv:HTTP404 · GetiriEndeksleri_ReturnIndices.csv:HTTP404

### Bülten keşfi (§250k) — ✓ thb202608071.zip indi · 295KB
- içerik: thb202608071.csv
- endeks izi: `thb202608071.csv → TARIH;ISLEM  KODU;BULTEN ADI;PAZAR GRUBU;PAZAR;YAPISAL BAZDA PIYASA ALT BOLUMU;ENSTRUMAN GRUBU;ENSTRUMAN TIPI;ENSTRUMAN SINIFI;ISLEM YONTEMI;PIYASA YAPICI;BIST 100 ENDEKS;BIST 30 ENDEKS;BRUT TAKAS;OZS`


---
**Sonuç:** XK100 ağırlıkları · XKTUM ağırlıkları · XKTMT ağırlıkları · Multiple fiyatları (136 fiyat) · Model sicili (39 fiyat) · risk metrikleri (93) · katılım fonları (getiri-modu 36) · bilanço tetiği (69) · endeks üyelikleri · endeks arşivi